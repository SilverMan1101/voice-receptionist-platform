...
from knowledge_service.main import app, get_db
...
def test_query_knowledge(monkeypatch):
    with TestClient(app) as client:
        ...
        knowledge_main.qdrant.search = MagicMock(return_value=[
            (0.85, {"text": "This is grounded info", "metadata": {}}, True),
            (0.60, {"text": "Less confident info", "metadata": {}}, False)
        ])
        knowledge_main.embedder.embed_text = MagicMock(return_value=[0.1] * 1536)
        ...
        knowledge_main.qdrant.search.assert_called_once()
        kwargs = knowledge_main.qdrant.search.call_args[1]
